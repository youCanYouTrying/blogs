...
项目默认使用**浅色主题**作为首选外观，深色模式作为可选切换项（通过 `next-themes` 实现）。

#### 浅色主题配色体系

| 用途           | 色值 / 描述                                                        |
| -------------- | ------------------------------------------------------------------ |
| 页面背景       | `#FAFAF9` 或 `stone-50`，温暖的米白色，避免纯白 `#FFFFFF` 的刺眼感 |
| 卡片 / 容器背景 | `#FFFFFF`，搭配极淡的投影 `shadow-sm` 实现层次                     |
| 主文字         | `#1C1917` 或 `stone-900`，接近纯黑但不刺眼                        |
| 次要文字       | `#78716C` 或 `stone-500`，用于日期、摘要、辅助信息                 |
| 主题强调色     | 一个低饱和度的色调（如暖棕 `#A16207`、深青 `#0F766E`、靛蓝 `#4338CA`），根据博客整体调性选定后全局统一使用，**不要用高饱和度的荧光色或渐变色** |
| 分割线 / 边框  | `#E7E5E4` 或 `stone-200`，极淡存在感                              |
| 链接 / 交互元素 | 主题强调色同色系，hover 时加深一个色阶即可                         |

#### 字体方案

- 正文优先使用系统原生字体栈：`-apple-system, BlinkMacSystemFont, "Segoe UI", "Noto Sans SC", sans-serif`
- 标题可搭配一款有辨识度但不花哨的字体（如 `Noto Serif SC` 或 `LXGW WenKai`）
- **禁止**使用纯装饰性英文花体字或极细字重（`font-weight < 300`），保证中文阅读舒适度

### 设计风格：去 AI 味，追求自然与克制

> [!IMPORTANT]
> 本项目的视觉设计**严禁**出现典型的 AI 生成页面特征。设计应当像**一个有审美的人类开发者手工打磨**的作品，而非 AI 批量产出的模板。

#### 🚫 禁止的设计元素（典型 AI 味特征）

- ❌ **渐变滥用**：背景大面积紫蓝渐变、文字渐变色、按钮彩虹渐变
- ❌ **霓虹光效**：发光文字（`text-shadow` glow）、边框发光、荧光色点缀
- ❌ **玻璃拟态过度**：全页面大面积毛玻璃 `backdrop-blur`、半透明叠加层过多
- ❌ **科技感装饰**：网格线背景、粒子动画、浮动几何图形、光标跟踪光晕
- ❌ **过度圆角**：所有元素 `rounded-2xl` 以上的圆润泡泡风格
- ❌ **暗色主力调**：以深色 / 黑色为基调的"高级感"风格（我们是浅色主题）
- ❌ **动画过量**：每个元素都有入场动画、hover 都有 scale 变换、无处不在的 transition
- ❌ **空洞的大字标语**：Hero 区域只有一句口号和一个渐变背景，没有实质内容
- ❌ **图标堆砌**：每个功能项前面都放一个 emoji 或 icon 做装饰

#### ✅ 推荐的设计方向

- ✅ **留白充足**：内容区域不超过 `max-w-3xl`（约 768px），两侧大量留白，阅读不压迫
- ✅ **层次靠排版而非颜色**：用字号大小、字重粗细、间距疏密来建立信息层级，不依赖彩色背景块
- ✅ **配色克制**：全站主色不超过 2 种（强调色 + 中性色），不使用超过 3 种彩色
- ✅ **交互含蓄**：hover 效果只做颜色微调或下划线出现，不做缩放、阴影剧变、颜色突变
- ✅ **真实内容驱动**：首页直接展示文章列表，不需要巨型 Banner 或空洞的 Slogan 占满首屏
- ✅ **细节质感**：文字行高 `1.75 ~ 1.85`，段落间距舒适，代码块用浅灰底色 + 等宽字体
- ✅ **印刷品质感**：整体参考优质博客（如 [overreacted.io](https://overreacted.io)、[antfu.me](https://antfu.me)、[leerob.io](https://leerob.io)）的排版节奏
- ✅ **自然的视觉节奏**：页面从上到下有明确的内容结构（导航 → 文章列表 → 页脚），不做花哨的分区装饰

#### 组件具体要求

| 组件           | 要求                                                                                              |
| -------------- | ------------------------------------------------------------------------------------------------- |
| 导航栏         | 简洁一行：左侧博客名（文字即可，无 Logo 图），右侧 3-4 个页面链接，不需要搜索框和主题切换图标占位 |
| Hero 区域      | 可以有，但不要做成全屏 Banner；用 2-3 行简短的自我介绍文字即可，GSAP 动画限于文字入场              |
| 文章卡片       | 标题 + 日期 + 摘要（可选标签），无封面图也要好看；卡片之间用分割线或间距区分，不用阴影厚重的卡片   |
| 文章详情页     | `prose` 排版为主，TOC 目录侧边栏可折叠，代码块样式简洁（浅色底 + 左侧语言标签）                   |
| 页脚           | 一行版权信息 + 社交链接（文字链接即可），不做多列复杂布局                                          |
